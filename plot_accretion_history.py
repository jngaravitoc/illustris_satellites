...
def get_analogues(sat1_lbt, sat2_lbt, sat3_lbt, mrat1, mrat2, mrat3, id_halos, analogue, write_data=False):
    ...
    if analogue == "LMC":
        mw_analogues = np.where(((sat1_lbt < LMC+0.7) & (sat1_lbt > LMC-0.7)))
        filename = "LMC_analogues.txt"

    elif analogue == "LMC+SAG":
        mw_analogues = np.where(((sat1_lbt < LMC+0.7) & (sat1_lbt > LMC-0.7)) & ((sat2_lbt < SAG+1) & (sat2_lbt>SAG-1)))
        filename = "LMC_SAG_analogues.txt"

    elif analogue == "LMC+GSE":
        mw_analogues = np.where(((sat1_lbt < LMC+0.7) & (sat1_lbt > LMC-0.7)) & ((sat3_lbt < GSE+1) & (sat3_lbt > GSE-1)))
        filename = "LMC_GSE_analogues.txt"
    
    elif analogue == "LMC+SAG+GSE":
        mw_analogues = np.where(((sat1_lbt < LMC+0.7) & (sat1_lbt > LMC-0.7)) & ((sat2_lbt < SAG+1) & (sat2_lbt>SAG-1)) & ((sat3_lbt < GSE+1) & (sat3_lbt > GSE-1)))
        filename = "LMC_SAG_GSE_analogues.txt"
    
    elif analogue == "SAG":
        mw_analogues = np.where(((sat2_lbt < SAG+1) & (sat2_lbt>SAG-1)))
        filename = "SAG_analogues.txt"
    
    elif analogue == "SAG+GSE":
        mw_analogues = np.where(((sat2_lbt < SAG+1) & (sat2_lbt>SAG-1)) & ((sat3_lbt < GSE+1) & (sat3_lbt > GSE-1)))
        filename = "SAG_GSE_analogues.txt"
    
    elif analogue == "GSE":
        mw_analogues = np.where(((sat3_lbt < GSE+1) & (sat3_lbt > GSE-1)))
    ...
    mratios = [mrat1[mw_analogues], mrat2[mw_analogues], mrat3[mw_analogues]]
   

    if write_data==True :         
        head = "ID host halo, mass ratio sat1, mass ratio sat 2, mass ratio sat 3, looback t sat1, lookback t sat 2, lookback sat 3"
        np.savetxt(filename, np.array([IDhalos, mratios[0], mratios[1],
...
mw_analogues = np.loadtxt("halos_three_mergers.txt")
...
#GSE_analogues = get_analogues(sat1lbt, sat2lbt, sat3lbt, sat1_mrat, sat2_mrat, sat3_mrat, GroupFirstSub, "GSE")
...
plt.savefig("histograms_lookback_times_satellites.png", bbox_inches='tight')
plt.close()
"""
